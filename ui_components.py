...
def draw_rotating_reticle(overlay, cx, cy, radius, angle, color, thickness=1):
    """Vectorized premium multi-ring targeting reticle."""
    # Inner solid ring
    cv2.circle(overlay, (cx, cy), int(radius * 0.55), CYAN_DIM, 1, cv2.LINE_AA)
    
    # Vectorized Cardinal Ticks (4)
    rad_a = math.radians(angle)
    c_angles = np.array([rad_a, rad_a + math.pi/2, rad_a + math.pi, rad_a + 3*math.pi/2])
    cos_a, sin_a = np.cos(c_angles), np.sin(c_angles)
    p1 = np.stack((cx + (radius-14)*cos_a, cy + (radius-14)*sin_a), axis=1).astype(np.int32)
    p2 = np.stack((cx + radius*cos_a, cy + radius*sin_a), axis=1).astype(np.int32)
    cardinal_segs = np.stack((p1, p2), axis=1)
    cv2.polylines(overlay, cardinal_segs, False, CYAN_BRIGHT, 2, cv2.LINE_AA)
    for i in range(4): cv2.circle(overlay, tuple(p2[i]), 2, WHITE_HOT, -1, cv2.LINE_AA)

    # Vectorized Fine Ticks (24)
    f_angles = rad_a + np.linspace(0, math.tau, 24, endpoint=False)
    cos_f, sin_f = np.cos(f_angles), np.sin(f_angles)
    f1 = np.stack((cx + (radius-5)*cos_f, cy + (radius-5)*sin_f), axis=1).astype(np.int32)
    f2 = np.stack((cx + radius*cos_f, cy + radius*sin_f), axis=1).astype(np.int32)
    fine_segs = np.stack((f1, f2), axis=1)
    cv2.polylines(overlay, fine_segs, False, CYAN_DIM, 1, cv2.LINE_AA)
