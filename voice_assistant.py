...
class VoiceAssistant:
    ...
    def _parse_command(self, text):
        ...
        if any(w in text for w in ["suit up", "deploy", "shoot up", "set up", "sweet up", "straight up"]):
            ...
            cmd = "set_mode_4"
        elif any(w in text for w in ["ultron", "purple"]):
            cmd = "set_mode_5"
        elif any(w in text for w in ["faceless", "void", "cloak", "invisible", "behind me"]):
            cmd = "set_mode_6"
        elif "mode" in text:
            ...
            cmd = "toggle_hud"
        elif any(w in text for w in ["save room", "empty room", "take photo", "take picture", "capture room"]):
            cmd = "save_room"
        elif any(w in text for w in ["cameraman", "auto frame", "center stage", "follow me", "camera man"]):
            cmd = "toggle_cameraman"
        ...
        if cmd:
            ...
            if cmd == "suit_up":
                ...
                self.speak("Empty room reference saved, sir.")
            elif cmd == "toggle_cameraman":
                self.speak("Toggling A I Cameraman.")
