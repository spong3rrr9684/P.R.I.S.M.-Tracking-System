...
def _vec_ticks(cx, cy, r, r2, angle_base_rad, angles_template, out_buf=None):
    """
...
class HUDModeStrategy:
    ...
    def render(self, overlay, face_landmarks, w, h, t, pts, nose, lines, dist, state, *args, **kwargs):
...
class NanoMode(HUDModeStrategy):
    ...
    _tick_buf = np.zeros((12, 2, 2), dtype=np.int32)
    ...
    def render(self, overlay, face_landmarks, w, h, t, pts, nose, lines, dist, state, *args, **kwargs):
        ...
        ticks = _vec_ticks(cx_f, cy_f, span, span + 5, math.radians(t * 20), _TICK12_ANGLES, self._tick_buf)
...
class QuantumMode(HUDModeStrategy):
    ...
    _tick_buf = np.zeros((18, 2, 2), dtype=np.int32)
...
class NeonCipherMode(HUDModeStrategy):
    _tick_buf = np.zeros((24, 2, 2), dtype=np.int32)
    ...
    def render(self, overlay, face_landmarks, w, h, t, pts, nose, lines, dist, state, *args, **kwargs):
        ...
        ticks = _vec_ticks(cx_f, cy_f, span + 22, span + 26, math.radians(t * 15), _TICK24_ANGLES, self._tick_buf)
...
class EdithMode(HUDModeStrategy):
    _tick_buf_inner = np.zeros((16, 2, 2), dtype=np.int32)
    _tick_buf_outer = np.zeros((36, 2, 2), dtype=np.int32)
    ...
    def render(self, overlay, face_landmarks, w, h, t, pts, nose, lines, dist, state, *args, **kwargs):
        ...
        for eye_idx in (468, 473):
            ...
            ticks = _vec_ticks(ex, ey, r_b, r_b + 3, math.radians(t * 25), _TICK16_ANGLES, self._tick_buf_inner)
        ...
        outer_ticks = _vec_ticks(cx_f, cy_f, span, span + 5, math.radians(t * -8), _TICK36_ANGLES, self._tick_buf_outer)
...
class UltronMode(HUDModeStrategy):
    ...
    def render(self, overlay, face_landmarks, w, h, t, pts, nose, lines, dist, state, *args, **kwargs):
        ...
        ticks = _vec_ticks(cx_f, cy_f, span+10, span+14, math.radians(-t*14), _TICK18_ANGLES, self._tick_buf)
...
# ─────────────────────────────────────────────────────────────────────────────
#  MODE 6 — FACELESS VOID
# ─────────────────────────────────────────────────────────────────────────────
class FacelessVoidMode(HUDModeStrategy):
    def render(self, overlay, face_landmarks, w, h, t, pts, nose, lines, dist, state, *args, **kwargs):
        image = kwargs.get('image', None)
        if image is None and len(args) > 0 and isinstance(args[0], np.ndarray):
            image = args[0]
        
        if image is not None and pts is not None and len(pts) >= 468:
            # 1. Create soft alpha mask of the face
            hull = cv2.convexHull(pts[:468])
            mask = np.zeros((h, w), dtype=np.float32)
            cv2.fillConvexPoly(mask, hull, 1.0)
            
            # Dilate slightly to cover jaw/hairline edges
            kernel = np.ones((15, 15), np.uint8)
            mask_dilated = cv2.dilate(mask, kernel, iterations=1)
            cv2.GaussianBlur(mask_dilated, (25, 25), 0, dst=mask_dilated)
            mask_3c = np.stack([mask_dilated]*3, axis=-1)
            
            # 2. Check if we have a background frame, otherwise use fast ROI inpainting
            bg = getattr(state, 'bg_frame', None)
            if bg is not None and getattr(bg, 'shape', None) == image.shape:
                image[:] = image * (1.0 - mask_3c) + bg * mask_3c
            else:
                # Fast ROI inpainting
                min_x = max(0, int(pts[:,0].min()) - 20)
                max_x = min(w, int(pts[:,0].max()) + 20)
                min_y = max(0, int(pts[:,1].min()) - 20)
                max_y = min(h, int(pts[:,1].max()) + 20)
                
                roi_img = image[min_y:max_y, min_x:max_x]
                roi_mask = (mask_dilated[min_y:max_y, min_x:max_x] * 255).astype(np.uint8)
                if roi_img.size > 0 and roi_mask.size > 0 and roi_img.shape[:2] == roi_mask.shape:
                    try:
                        inpainted = cv2.inpaint(roi_img, roi_mask, 5, cv2.INPAINT_TELEA)
                        image[min_y:max_y, min_x:max_x] = inpainted
                    except Exception:
                        pass
            
            # 3. Add a subtle chromatic shimmer / refraction ring on the perimeter of the cloak
            perimeter = np.zeros((h, w), dtype=np.uint8)
            cv2.polylines(perimeter, [hull], True, 255, 2, cv2.LINE_AA)
            cv2.GaussianBlur(perimeter, (5, 5), 0, dst=perimeter)
            perim_idx = perimeter > 30
            # Subtle cyan/purple cloaking edge glow
            image[perim_idx] = cv2.addWeighted(image[perim_idx], 0.6, np.full_like(image[perim_idx], (255, 150, 50)), 0.4, 0)

        return (255, 180, 50), "FACELESS // VOID"
